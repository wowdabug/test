...
class WorldLayer extends UiLayer
{
    ...
    constructor(boundingBox)
    {
        ...
        for(var i = 0; i < tradeConfig.tradingPosts.length; ++i)
        {
            this.addHitbox(new WorldEntityHitbox(
                ...
                },
                {
                    ...
                    {
                        if(tradeConfig.tradingPosts[i].structure.level == 0)
                        {
        ...
        this.addHitbox(new WorldEntityHitbox(10, {
            ...
            onmousedown: function ()
            {
                ...
                {
                    chestService.grantChest(0, Chest.purchased, ChestType.black)
        ...
        //Super Miners Hitboxes
        if(this.allowSuperMinersHitboxes)
        {
            superMinerManager.currentSuperMiners.forEach(superminer =>
            {
                if(superminer.superMinerHitbox)
                {
                    this.addHitbox(superminer.superMinerHitbox);
                }
            });
        }
        ...
        if(hasFoundUfo == 0)
        {
            var ufoHitbox = new WorldEntityHitbox(1019, {
                x: (this.boundingBox.width / uiScaleX) * .1,
                y: (this.boundingBox.height / uiScaleY) * .025,
                width: worldUfo.width / 4,
                height: worldUfo.height * 1.5
            }, {
                onmousedown: function ()
                {
                    if(isUfoVisible())
            ...
            ufoHitbox.render = function ()
            {
                if(currentlyViewedDepth > 1018 && currentlyViewedDepth < 1030)
                {
                    if(isUfoVisible())
                    {
                        var coords = this.getGlobalCoordinates(0, 0);
                        var ufoPositionX = Math.ceil(mainw * .35) + Math.floor(oscillate(numFramesRendered, 79) * Math.ceil(mainw * .33));
                        var ufoPositionY = coords.y + Math.floor(oscillate(numFramesRendered, 31) * Math.ceil(mainw * .05));
                        MAIN.drawImage(worldUfo, (worldUfo.width / 4) * getAnimationFrameIndex(3, 10), 0, worldUfo.width / 4, worldUfo.height, ufoPositionX, ufoPositionY, Math.floor(mainw * .075), Math.floor(mainh * .14));

                        this.boundingBox.x = ufoPositionX;
                    }
                }
            }

            this.addHitbox(ufoHitbox);
        }
    }
    ...
    renderTopCity()
    {
        if(currentlyViewedDepth < 4)
        {
            ...
            MAIN.font = "900 " + MAIN.canvas.width * 0.0125 + "px KanitM";
            ...
            if(language == "german") MAIN.font = MAIN.canvas.width * 0.011 + "px KanitM";
            if(language == "spanish") MAIN.font = MAIN.canvas.width * 0.011 + "px KanitM";
            if(language == "japanese") MAIN.font = MAIN.canvas.width * 0.011 + "px KanitM";
            if(language == "french") MAIN.font = MAIN.canvas.width * 0.011 + "px KanitM";
            ...
            MAIN.font = "900 " + MAIN.canvas.width * 0.011 + "px KanitM";
            if(language == "french") MAIN.font = MAIN.canvas.width * 0.012 + "px KanitM";
            ...
            if(currentlyViewedDepth < 3)
            {
                ...
                MAIN.lineWidth = 2;
                MAIN.font = MAIN.canvas.width * 0.0125 + "px KanitM";
            ...
            if(chestCollectorChanceStructure.level > 0 && chestCollectorStorageStructure.level > 0)
            {
                ...
                MAIN.font = MAIN.canvas.width * 0.015 + "px Verdana";
            ...
            if(currentlyViewedDepth == 0)
            {
                ...
                if(limitedTimeEventManager.isEventActive)
                {
                    if(limitedTimeEventManager.activeEvent.displayCountdown)
                    {
                        ...
                        MAIN.save();
                        MAIN.font = MAIN.canvas.width * 0.01 + "px Verdana";
                        MAIN.fillStyle = "#FFF";
    ...
    renderTradingPost()
    {
        for(var i = 0; i < tradeConfig.tradingPosts.length; ++i)
        {
            if(currentlyViewedDepth >= tradeConfig.tradingPosts[i].depth && currentlyViewedDepth < tradeConfig.tradingPosts[i].depth + 10)
            {
                ...
                MAIN.font = MAIN.canvas.width * 0.01 + "px KanitM";
                if(tradeConfig.tradingPosts[i].structure.level != 0)
                {
    ...
    renderUndergroundCity()
    {
        if(currentlyViewedDepth > 299 && currentlyViewedDepth < 309)
        {
            ...
            MAIN.save();
            if(currentTargetHitbox && currentTargetHitbox.hitbox && currentTargetHitbox.hitbox.id == "weaponCrafting")
            {
                MAIN.shadowBlur = 8;
            ...
            MAIN.save();
            if(currentTargetHitbox && currentTargetHitbox.hitbox && currentTargetHitbox.hitbox.id == "oilRig")
            {
                MAIN.shadowBlur = 8;
                MAIN.shadowColor = "rgba(255, 255, 64, .5)";
            }
            ...
            MAIN.save();
            if(currentTargetHitbox && currentTargetHitbox.hitbox && currentTargetHitbox.hitbox.id == "gemForge")
            {
                MAIN.shadowBlur = 8;
                MAIN.shadowColor = "rgba(255, 255, 64, .5)";
            }
    ...
    renderEndOfWorld()
    {
        ...
        if(currentlyViewedDepth > 1019 && currentlyViewedDepth < 1029)
        {
            ...
            //     var ufoPositionX = Math.ceil(mainw * .35) + Math.floor(oscillate(numFramesRendered, 79) * Math.ceil(mainw * .33));
            //     var ufoPositionY = Math.ceil(mainh * .17) + Math.floor(oscillate(numFramesRendered, 31) * Math.ceil(mainw * .05));
            //     MAIN.drawImage(worldUfo, (worldUfo.width / 4) * getAnimationFrameIndex(3, 10), 0, worldUfo.width / 4, worldUfo.height, ufoPositionX, yCoordinateOfLevelTop + ufoPositionY, Math.floor(mainw * .075), Math.floor(mainh * .14));
            //     var ufoUpdatedHitbox = this.getHitboxById("ufoClickable");
            //     ufoUpdatedHitbox.boundingBox.x = ufoPositionX;
            //     ufoUpdatedHitbox.boundingBox.y = ((mainh * .111 + ((ufoUpdatedHitbox.kmDepth + 4) * .178 * mainh)) / uiScaleY) + ufoPositionY;
            // }
        }
    ...
    renderMoonbase()
    {
        if(currentlyViewedDepth > 1027 && currentlyViewedDepth < 1037)
        {
            ...
            if(language == "german") MAIN.font = MAIN.canvas.width * 0.01 + "px KanitM";
            if(language == "spanish") MAIN.font = MAIN.canvas.width * 0.01 + "px KanitM";
            if(language == "japanese") MAIN.font = MAIN.canvas.width * 0.01 + "px KanitM";
            if(language == "french") MAIN.font = MAIN.canvas.width * 0.01 + "px KanitM";
    ...
    {
        if(battleManager.battleWaiting.length > 0 && battleManager.battleWaiting[0][1] == depthToRender)
        {
    ...
    renderLevelImageForDepth(depthToRender, xCoordinateOfLevel, yCoordinateOfLevelTop)
    {
        ...
        else if(depth >= 1000)
        {
            ...
            else if(!battleManager.isActiveBossLevel(depthToRender))
            {
                ...
                else if(depthToRender < 2014)
                {
                    MAIN.drawImage(titanlevel5, 0, 0, 926, 120, xCoordinateOfLevel, yCoordinateOfLevelTop, Math.floor(mainw * .724), Math.ceil(mainh * .178));
                }
                else if(depthToRender < 2114)
                {
                    MAIN.drawImage(titanlevel6, 0, 0, 926, 120, xCoordinateOfLevel, yCoordinateOfLevelTop, Math.floor(mainw * .724), Math.ceil(mainh * .178));
                }
                else if(depthToRender < 2214)
                {
                    MAIN.drawImage(titanlevel7, 0, 0, 926, 120, xCoordinateOfLevel, yCoordinateOfLevelTop, Math.floor(mainw * .724), Math.ceil(mainh * .178));
                }
                else if(depthToRender < 2314)
                {
                    MAIN.drawImage(titanlevel8, 0, 0, 926, 120, xCoordinateOfLevel, yCoordinateOfLevelTop, Math.floor(mainw * .724), Math.ceil(mainh * .178));
                }
    ...
    renderKmTextForDepth(depthToRender, xCoordinateOfLevel, yCoordinateOfLevelTop)
    {
        MAIN.save();
        MAIN.fillStyle = "#FFFFFF";
        MAIN.font = MAIN.canvas.width * 0.0125 + "px CFont";
        MAIN.lineWidth = 3;
        if(depthToRender < 1032)
        {
    ...
    renderWorkersForDepth(depthToRender, yCoordinateOfLevelTop)
    {
            ...
            for(var i = chestService.getChestsAtDepth(depthToRender).length; i > 0; i--)
            {
                ...
                else if(chest.type == ChestType.battle)
                {
                    if(depthToRender < 1000)
                    {
                        if(workerLevel < 5)
                        {
                            MAIN.drawImage(foundBattle1, 32 * getAnimationFrameIndex(4, 10), 0, 32, 96, Math.ceil(mainw * (.085 + ((chest.worker - 1) * .072))), yCoordinateOfLevelTop, Math.ceil(mainw * .025), Math.floor(mainh * .142));
                        }
                        else
                        {
                            MAIN.drawImage(window["foundBattle" + workerLevel], 32 * getAnimationFrameIndex(4, 10), 0, 32, 96, Math.ceil(mainw * (.085 + ((chest.worker - 1) * .072))), yCoordinateOfLevelTop, Math.ceil(mainw * .025), Math.floor(mainh * .142));
                        }
                    }
                    else
                    {
                        MAIN.drawImage(foundBattlem, 32 * getAnimationFrameIndex(4, 10), 0, 32, 96, Math.ceil(mainw * (.085 + ((chest.worker - 1) * .072))), yCoordinateOfLevelTop, Math.ceil(mainw * .025), Math.floor(mainh * .142));
                    }
                }
                else
                {
        ...
        let workerY = yCoordinateOfLevelTop + Math.floor(mainh * .075);
        let workerWidth = Math.ceil(mainw * .025);
        let workerHeight = Math.floor(mainh * .071);
        ...
        for(var m = 1; m <= workersToDisplay; m++)
        {
            if(!workersToSkip.includes(m))
            {
                ...
                let inBattle = battleManager.battleWaiting.length > 0 &&
                    m == battleManager.battleWaiting[0][0] &&
                    battleManager.battleWaiting[0][1] == depthToRender;

                let beingAttackedX = 0;
                let attackFrame = numFramesRendered % 30;

                if(inBattle)
                {
                    let intensity = Math.random() * 0.0005 - 0.00025;
                    if(attackFrame >= 10 && attackFrame < 15)
                    {
                        beingAttackedX = intensity;
                    } else if(attackFrame >= 20 && attackFrame < 25)
                    {
                        beingAttackedX = intensity * 2;
                    } else if(attackFrame >= 25)
                    {
                        beingAttackedX = intensity * -1.5;
                    }
                }

                let minerX = Math.ceil(mainw * (.085 + ((m - 1) * (.072 + beingAttackedX))));
                ...
                if(!isCapacityFull())
                {
                    ...
                    else
                    {
                        ...
                        if(depthToRender < 1000 && minerHatImages[workerLevel].id != "bigtransblock")
                        {
                ...
                { //no animation, not mining
                    if(depthToRender < 1000)
                    {
                        MAIN.drawImage(minerImages[workerLevel], 0, 0, 32, 48, minerX, yCoordinateOfLevelTop + Math.floor(mainh * .075), Math.ceil(mainw * .025), Math.floor(mainh * .071));
                        MAIN.drawImage(minerHatImages[workerLevel], 0, 0, 32, 48, minerX, yCoordinateOfLevelTop + Math.floor(mainh * .075), Math.ceil(mainw * .025), Math.floor(mainh * .071));
                    }
                    else if(depthToRender <= 1782)
                    {
                        if(m == 8 && depthToRender == 1089) //render jeb, c'mon jeb what are you doing...
                        {
                            drawImageRot(MAIN, lunarMinerImages[workerLevel], 0, 0, 32, 48, minerX, yCoordinateOfLevelTop + Math.floor(mainh * .075), Math.ceil(mainw * .025), Math.floor(mainh * .071), 180);
                        }
                        else
                        {
                            MAIN.drawImage(lunarMinerImages[workerLevel], 0, 0, 32, 48, minerX, yCoordinateOfLevelTop + Math.floor(mainh * .075), Math.ceil(mainw * .025), Math.floor(mainh * .071));
                        }
                    }
                    else if(depthToRender >= getTitan().startDepth)
                    {
                        MAIN.drawImage(titanMinerImages[workerLevel], 0, 0, 32, 48, minerX, yCoordinateOfLevelTop + Math.floor(mainh * .075), Math.ceil(mainw * .025), Math.floor(mainh * .071));
                    }
                }
                ...
                if(inBattle)
                {
                    ...
                    MAIN.drawImage(Exclamation,
                        0,
                        0,
                        Exclamation.width,
                        Exclamation.height,
                        Math.ceil(mainw * (.091 + ((m - 1) * .072))),
                        yCoordinateOfLevelTop + Math.floor(mainh * (.045 - (.01 * oscillate(numFramesRendered, 10)))),
                        Math.ceil(mainw * .02),
                        Math.floor(mainh * .04)
                    );
                }
