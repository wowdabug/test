...
const TRADE_TYPE_RELIC_SCRAP = 7;
...
const tradeConfig = {
    tradingPosts: [
        {
            depth: worlds[EARTH_INDEX].tradePostDepth,
            ...
            structure: tradingPostStructure
        },
        {
            depth: worlds[MOON_INDEX].tradePostDepth,
            image: lunarTradingPostLevel,
            unbuiltImage: lunarTradingPostLevelBroken,
            buildingTextCoords: new ScaledPosition(.291, .046),
            playerHasSeenNewTrade: false,
            structure: moonTradingPostStructure
        },
        {
            depth: worlds[TITAN_INDEX].tradePostDepth,
            image: titanTradingPostLevel,
            unbuiltImage: titanTradingPostLevelBroken,
            buildingTextCoords: new ScaledPosition(.304, .046),
            playerHasSeenNewTrade: false,
            structure: titanTradingPostStructure
        }
    ],
...
var moonTradeOffer1 = [];
var moonTradeOffer2 = [];
var timeBetweenTradesTestMultiplier = 0.9;

var nextTradeTimeTitan = 0;
var titanTradeOffer1 = [];
var titanTradeOffer2 = [];
...
function checkForNewTrade()
{
    for(var i = 0; i < tradeConfig.tradingPosts.length; ++i)
    {
        if(depth >= tradeConfig.tradingPosts[0].depth &&
            tradeConfig.tradingPosts[i].structure.level > 0 &&
...
function getTradesForWorld(worldIndex)
{
    ...
    switch(parseInt(worldIndex))
    {
        ...
        case 2:
            return [titanTradeOffer1, titanTradeOffer2];
        default:
...
function getNextTradeTimeForWorld(worldIndex)
{
    ...
    switch(parseInt(worldIndex))
    {
        ...
        case 2:
            if(isNaN(nextTradeTimeTitan)) nextTradeTimeTitan = 0;
            return nextTradeTimeTitan;
        default:
            throw "Error: No trades exist for World " + worldIndex;
    }
}
...
{
    var leveledStatsValue = tradeConfig.tradingPosts[worldIndex].structure.statValueForCurrentLevel();
...
{
    var traderList = filterRelicScrapTraders(worldIndex);
    return tradeRoller.rand(0, traderList.length - 1);
}

function filterRelicScrapTraders(worldIndex)
{
    var filteredTraders = traders[worldIndex];

    if(worldIndex == EARTH_INDEX && !hasScrappedFirstRelic)
    {
        // If the player hasn't scrapped a relic yet, don't allow relic scrap trades
        filteredTraders = filteredTraders.filter(trader =>
        {
            return !trader.paymentTypes.some(paymentType => paymentType.type == TRADE_TYPE_RELIC_SCRAP);
        });
    } else if(worldIndex == EARTH_INDEX && hasScrappedFirstRelic && Math.random() > 0.5)
    {
        // If the player has scrapped a relic, there is a 50% chance that the scrap trader will be included in the list
        filteredTraders = filteredTraders.filter(trader =>
        {
            return !trader.paymentTypes.some(paymentType => paymentType.type == TRADE_TYPE_RELIC_SCRAP);
        });
...
function calculateAmountOfTradeType(tradeType, tradeSubtype, timeValue, isPayment, otherType = null, otherSubtype = null, otherAmount = null)
{
    ...
    switch(tradeType)
    {
        ...
        case TRADE_TYPE_RELIC_SCRAP:
            return new BigNumber(Math.ceil(timeValue / determineTimeValueOfTradeType(TRADE_TYPE_RELIC_SCRAP)));
        default:
            throw "Invalid trade type";
    }
}
...
function canMakeTrade(tradeOffer, showMessageOnFail = false)
{
    ...
    else if(tradeOffer[TRADE_INDEX_PAYMENT_TYPE] == TRADE_TYPE_RELIC_SCRAP)
    {
        canMakeTrade = isTradeAvailable(tradeOffer) &&
            numRelicScrapsOwned() >= tradeOffer[TRADE_INDEX_PAYMENT_AMOUNT];
        message = _("You can't afford that trade.");
    }
...
function makeTrade(worldIndex, tradeOffer)
{
    ...
    {
        removeRelicScrap(parseInt(tradeOffer[TRADE_INDEX_PAYMENT_AMOUNT]));
    }
...
function generateTradeOfferStrings(tradeOffer)
{
    ...
    switch(tradeOffer[TRADE_INDEX_PAYMENT_TYPE])
    {
        ...
        case TRADE_TYPE_RELIC_SCRAP:
            {
                paymentString = _("{0} Relic Scrap", tradeOffer[TRADE_INDEX_PAYMENT_AMOUNT]);
                paymentValueString = _("Unknown");
            }
    }
    switch(tradeOffer[TRADE_INDEX_REWARD_TYPE])
    {
...
var traders = [
    [
        ...
        {
            "id": 16,
            "name": _("Scraps McDiggs"),
            "portrait": trader9,
            "blinkPortrait": null,
            "introDialogue": [
                _("Only the finest bits of scrap catch my eye. Got somethin' rare?"),
                _("Scrap is my bread and butter... but these relics? They're the dessert."),
                _("Come back when you've got something that <em>shines</em>.")
            ],
            "paymentTypes": [
                {
                    type: TRADE_TYPE_RELIC_SCRAP,
                    probability: 50
                }
            ],
            "rewardTypes": [
...
function testFrequencyOfTradeReward(rewardType, testsToRun = 20, tradesPerTest = 100)
{
    let trades = {
        ...
        [TITAN_INDEX]: new Array(testsToRun).fill(0)
    };
    ...
    for(var x = 0; x < testsToRun; x++)
    {
        for(var i = 0; i < tradesPerTest; i++)
        {
            ...
            let titanTrades = getTradesForWorld(TITAN_INDEX);
            ...
            if(titanTrades[0][TRADE_INDEX_REWARD_TYPE] == rewardType) trades[MOON_INDEX][x]++;
            if(titanTrades[1][TRADE_INDEX_REWARD_TYPE] == rewardType) trades[MOON_INDEX][x]++;
        }
    }
    ...
    console.log("Titan Trades of Type: " + calculateAverageOfArray(trades[TITAN_INDEX]));
}