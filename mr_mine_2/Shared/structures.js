...
gemForgeStructure.leveledStats = [
    {
        name: _("Max Queue"),
        perLvlValue: [6, 10, 20, 30, 40, 55, 75, 85, 100, 120]
    }
]
...
oilrigStructure.leveledStats = [
    {
        name: _("Oil Capacity"),
        perLvlValue: [...oilRigStats].splice(1, oilRigStats.length).map(x => x[1])
    },
    {
        name: _("Generation Time"),
        perLvlValue: [...oilRigStats].splice(1, oilRigStats.length).map(x => {return _("{0}s", x[0])})
    }
]
...
moonTradingPostStructure.depthAvailable = worlds[MOON_INDEX].tradePostDepth;
...
chestCompressorStructure.leveledStats = [
    {
        ...
        perLvlValue: [15, 14, 12, 10, 9, 8, 7, 6]
    },
    {
...
var titanTradingPostStructure = new Structure(15, 0);
titanTradingPostStructure.translatedName = _("Titan Trading Post");
titanTradingPostStructure.maxLevel = 5;
titanTradingPostStructure.icon = icontptitan;
titanTradingPostStructure.structureDescription = [_("Trade for minerals, chests, and more."), ""];
titanTradingPostStructure.leveledStats = [
    {
        name: _("Trade Improvement"),
        perLvlValue: ["0%", "50%", "100%", "150%", "200%"]
    }
]
titanTradingPostStructure.depthAvailable = worlds[TITAN_INDEX].tradePostDepth;
structures.push(titanTradingPostStructure);
...
function learnReachedStructures()
{
    ...
    if(depth > 700 && chestCompressorSlotStructure.level < 1 && chestCompressorStructure.level < 1)
    {
...
function resetStructuresForAscension()
{
    structures.forEach(structure =>
    {
        structure.level = structure.startingLevel;
    })
}