...
function getWorldForDepth(depthToCheck)
{
    for(let world of worlds)
    {
        if(depthToCheck >= world.startDepth && depthToCheck <= world.endDepth)
        {
            return world;
        }
    }
...
function formattedDepth(depth)
{
    let world = getWorldForDepth(depth);
    return "W" + (world.index + 1) + "-" + (depth - world.startDepth) + "km";
}
...
moon.endDepth = 1813;
moon.tradePostDepth = moon.startDepth + 15;
...
titan.endDepth = 2589;
titan.tradePostDepth = titan.startDepth + 15;
...
function isDepthWithoutWorkers(depth)
{
    return depth < 0
        ...
        || (depth == tradeConfig.tradingPosts[TITAN_INDEX].depth)
}
