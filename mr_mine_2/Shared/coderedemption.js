...
async function fetchAndRedeemUserCoupons()
{
	var codeUrl = "https://mrmine.com/SaveRestoration/usercoupons.php";
	var atomicId = Playsaurus.getAtomicId();
	ajax(
		codeUrl,
		{
			userId: atomicId
		...
		function (response)
		{
			...
			if(response.success)
			{
				for(var i in response.data.codes)
				{
					redeemCouponThroughSdk(response.data.codes[i], true);
				}
			}
		}
	);
