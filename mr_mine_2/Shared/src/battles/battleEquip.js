...
class weaponDrawEquip extends BattleEquip
{
    ...
    onUse()
    {
        battleManager.drawWeapons(0, this.getCardDraw());
    }

    canUse()
    {
        return super.canUse() && battleManager.atkWeps.length > 0 &&
            battleManager.activeWeps.length <= battleManager.absoluteCardCap;
    }
    ...
    getCardDraw()
    {
        let cardsAllowedToDraw = battleManager.absoluteCardCap - battleManager.activeWeps.length;
    ...
    getDescription()
    {
        return _("Card Draw: {0}", beautifynum(this.getCardDraw()));
    }
    ...
    drawDescriptionForLevel(context, x, y, maxWidth, level)
    {
        ...
        fillTextShrinkToFit(
            context,
            _("Card Draw: {0}", beautifynum(this.getCardDrawForLevel(level))),
            x,
            y,
            maxWidth
        );
...
function grantDefaultEquips()
{
    ...
    battleEquipsManager.slots = battleEquipsManager.activeEquips.length + 2;
}