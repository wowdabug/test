class CaveSystem
{
    ...
    //############# CAVE COUNSUMABLES METHODS #################

    revealCave()
    {
        if(!this.isFullyRevealed())
        {
            this.revealAllNodeChildren(this.rootNode);
            newNews(_("The cave at {0}km has been revealed!", this.kmDepth));
        }
        else
        {
            newNews(_("The cave at {0}km is already revealed!", this.kmDepth));
        }
    }

    revealAllNodeChildren(node = null)
    {
        if(!node) node = this.rootNode;
        if(!node.isRevealed) node.isRevealed = true;
        this.getAllChildNodesFromRoot().forEach((node) => node.isRevealed = true)
    }

    sendDroneFleet()
    {
        if(!this.hasDroneFleet)
        {
            var discount = 0.5 //Fuel discount for fleet manager use

            //filters and sorts nodes that have rewards based on distance
            let nodes = this.getAllChildNodesFromRoot()
                .filter(node => this.getIndexOfRewardsOnNode(node).length > 0)
                .sort((a, b) => a.depth - b.depth);

            //Sends out drones until there is not enought fuel or there are no more reachable rewards
            //Fuel based on most expensive drone * discount
            while(this.currentFuel >= 75 && nodes.length > 0)
            {
                ...
                //Send drone at a discounted fuel cost
                var selectedPath = this.getPathToNode(selectedNode);
                this.startDroneOnDiscountedPath(drone, selectedPath, discount);
                nodes = nodes.filter(node => node != selectedNode);
            };
            this.hasDroneFleet = true;
            newNews(_("A drone fleet has been deployed at {0}km!", this.kmDepth));
        } else
        {
    ...
    startDroneOnDiscountedPath(baseDrone, nodePath, discount)
    {
        if(this.canCraftDrone(baseDrone))
        {
            this.currentFuel -= baseDrone.totalFuel * discount;
            var newDrone = new ActiveDrone(baseDrone, this, nodePath);
            this.activeDrones.push(newDrone);
            if(!this.isExplored)
            {
    ...
    getIndexOfRewardsOnNode(node)
    {
        ...
        for(var i in this.rewards)
        {
            if(this.rewards[i] && this.rewards[i].locationNode && this.rewards[i].locationNode == node)
            {
                rewards.push(i);
            }
        }
    ...
    selectWeightedRandomType(types)
    {
        ...
        // return first type if something goes wrong
