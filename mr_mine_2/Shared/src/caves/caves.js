...
const SECOND_CAVE_SPAWN_DEPTH = 100;
...
var hasSecondCaveSpawned = false;

var _droneDebug = false;
...
//###########################################################
var cachedCaveValues = {
    cachedAtDepth: -1,
    avgSecsBetweenSpawns: 0,
    maxConcurrentCaveSystems: 0,
    maxRoll: 0
}// Cached value

function rollForCaveSystemSpawn()
{
    if(depth != cachedCaveValues.cachedAtDepth)
    {
        cachedCaveValues.avgSecsBetweenSpawns = getNumSecondsBetweenSpawnsAtCurrentDepth();
        cachedCaveValues.maxConcurrentCaveSystems = Math.min(4, (depth - MIN_CAVE_SYSTEM_SPAWN_DEPTH) / 100);
        cachedCaveValues.cachedAtDepth = depth;
        cachedCaveValues.maxRoll = Math.floor(cachedCaveValues.avgSecsBetweenSpawns / CAVE_UPDATE_DELTA_TIME_SECONDS);
    }

    var canSpawnFirstCave = !hasFirstCaveSpawned && !isSimulating && depth >= MIN_CAVE_SYSTEM_SPAWN_DEPTH;
    var canSpawnSecondCave = hasFirstCaveSpawned && !hasSecondCaveSpawned && !isSimulating && depth >= SECOND_CAVE_SPAWN_DEPTH;

    if(canSpawnFirstCave || canSpawnSecondCave || (caveRoller.rand(0, cachedCaveValues.maxRoll) == 1 && numActiveCaves() < cachedCaveValues.maxConcurrentCaveSystems))
    {
        ...
        else if(canSpawnSecondCave)
        {
            depthToSpawn = SECOND_CAVE_SPAWN_DEPTH;
        }
...
function resetCavesForAscension()
{
    getActiveCaves().forEach(cave => cave.collapse());
    treasureStorage.treasure = [];
}

function switchDroneDebug()
{
    _droneDebug = !_droneDebug;
    console.log("Drone debug: " + _droneDebug);
}

function getDroneDebug()
{
    return _droneDebug;
}
...
var caveTips = [
    ...
    _("TIP: Magnetic drones won't pull treasure into hazardous locations during travel"),
    _("TIP: Magnetic drones will always pull treasure on the last location of their path")
];