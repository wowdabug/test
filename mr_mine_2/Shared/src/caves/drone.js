...
class ActiveDrone extends BaseDrone
{
    ...
    update(deltaTime)
    {
        if(this.logStatus && this.elapsedTime == 0) console.log("Drone " + this.id + " is starting path of length " + this.nodePath.length);
    ...
    turnAround()
    {
        //Drops rewards being collected and turns back
    ...
    startCollectingRewardsOnNode(node)
    {
        ...
        let singleDrone = this.caveSystem.getDronesOnNode(node).length == 0;
        for(var i = 0; i < rewards.length; ++i)
        {
            var reward = this.caveSystem.rewards[rewards[i]];
            if(singleDrone && !reward.canBeCollected(this)) reward.isClaimed = false;
...
magneticDrone.clearingObstacle = false;
magneticDrone.specialUpdate = function (deltaTime)
{
    if(this.claimedRewards.length > 0 && this.clearingObstacle === false)
    {
        for(var i in this.claimedRewards)
        {
...
{
    if((this.isMovingForward || this.lastReachedNodeIndex == this.nodePath.length - 1) && this.clearingObstacle === false)
    {
        for(var i in node.children)
        {
            if(node.children[i] != this.nodePath[this.nextNodeIndex()] &&   // Don't pull rewards from the node we're moving to
                (!(node instanceof CaveHazard) ||   //Pull rewards if we're on a safe node
                    (node instanceof CaveHazard) && //Pull rewards if we're on a hazard node and the reward is on a hazard, or if we are on the last node
                    (node.children[i] instanceof CaveHazard || this.lastReachedNodeIndex == this.nodePath.length - 1))) 
            {
...
magneticDrone.turnAround = function ()
{
    if(this.claimedRewards.length > 0)
    {
        for(var i in this.claimedRewards)
        {
            var reward = this.caveSystem.rewards[this.claimedRewards[i]];
            reward.isClaimed = false;
        }
        this.claimedRewards = [];
    }
