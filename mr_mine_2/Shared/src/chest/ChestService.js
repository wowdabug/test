class ChestService
{
    ...
    spawnRandomChests()
    {
        ...
        for(var x = 1; x <= maxTenthOfDepth; x++) //starts spawing chests after reaching 20km
        {
    ...
    spawnChest(spawnDepth, source = Chest.natural, type = ChestType.basic, miner = -1)
    {
        ...
        let chanceOfCollision = (this.chests.length + 1) / (Math.min(100, depth / 10)); //this is here to simulate how chest spawns acted in the old system.
    ...
    rollForSpecialChest()
    {
        ...
        let blackChestChance = randRoundToInteger(7900 * STAT.etherealChestSpawnFrequencyMultiplier()); //1 in 790
        let canSpawnBlackChest = chestSpawnRoller.rand(0, blackChestChance) <= 10;

        let battleChestChance = randRoundToInteger(3500); //1 in 350
        let canSpawnBattleChest = chestSpawnRoller.rand(0, battleChestChance) <= 10 && depth > 304;

        if(canSpawnBlackChest)
        {
            return ChestType.black;
        }
        else if(canSpawnBattleChest)
        {
            return ChestType.battle;
        }
    ...
    giveChestReward(chest, saveGameOnOpen = true, isInstantChest = false)
    {
        ...
        {
            this.chestRewardText = battleChestRewards.rollForRandomReward();
            this.totalBattleChestsOpened++;
        }
        ...
        if(saveGameOnOpen)
        {
            savegame("chest_opened");
        }
    }

    /**
     * Generates an instant chest of the specified type. Instant chests are not assigned to miners.
     * @param {ChestType.type} type Type of chest to generate. Default is ChestType.basic.
     * @returns {object} Instant chest object.
     */
    generateInstantChest(type = ChestType.basic)
    {
        return Chest.instant.new(Chest.instant, type);
    }

    /**
     * Spawns a chest and presents it to the player. If the chest cannot be spawned, the chest window opens with an instant chest.
     * @param {number} depth Depth at which the chest should be spawned. Default is 0.
     * @param {Chest.source} source Chest source. Default is Chest.purchased.
     * @param {ChestType.type} type Type of chest to spawn. Default is ChestType.basic.
     */
    grantChest(depth = 0, source = Chest.purchased, type = ChestType.basic)
    {
        if(this.spawnChest(depth, source, type))
        {
            this.presentChest(depth);
        }
        else
        {
            let instantChest = this.generateInstantChest(type);
            if(!isSimulating && !keysPressed["Shift"] && chestService.chestPopupEnabled)
            {
                openUiWithoutClosing(ChestWindow, null, instantChest);
            }
            else if(!activeLayers["Chest"])
            {
                this.giveChestReward(instantChest);
                newNews(_("You got {0} from a Chest!", chestService.getChestRewardText()), true);
            }
            trackEvent_FoundChest(type)
        }
    }
    ...
    grantStoredChest(chestType)
    {
        ...
        chestService.spawnChest(0, Chest.metaldetector, chestType, -1, false);
    ...
    testChestCollector(chestsToSpawn)
    {
        ...
        {
            chestsSpawned++;
            this.spawnChestAtRandomDepth(Chest.natural, false);
