    ...
    if(news.length > 0)
    {
        for(var i = 0; i < news.length; i++)
        {
            ...
            else
            {
                ...
                let textWidth = TransientEffectsC.measureText(news[i][0]).width;
                let textX = transientEffectContainerW * .5 - (textWidth / 2);
                let textY = transientEffectContainerH * ((.025 * (i + 1)) + .11);
                TransientEffectsC.strokeText(news[i][0], textX, textY);
                TransientEffectsC.globalAlpha = alpha;
                TransientEffectsC.lineWidth = 5;
                TransientEffectsC.strokeText(news[i][0], textX, textY);
                TransientEffectsC.fillText(news[i][0], textX, textY);
...
{
    var boundingBox;
    var isPremium = (text === premiumMinerMessage);
    var bubbleTailImage = (isPremium) ? speechBubbleTailGold : speechBubbleTail;
    var bubbleColor = (isPremium) ? "#ffe659" : "#FFFFFF";
    var textColor = (isPremium) ? "#7F4E08" : "#000000";
    ...
    if(isPremium)
    {
        ...
        context.shadowColor = "rgba(150, 150, 150, 0.75)";
    }
    renderRoundedRectangle(context, boundingBox.x1 + x - bubblePadding, boundingBox.y1 + y - bubblePadding - (boundingBox.height + (bubblePadding * 2)), boundingBox.width + (bubblePadding * 2), boundingBox.height + (bubblePadding * 2), 5, "#000000", bubbleColor, 1);
    context.restore();
    ...
    context.drawImage(bubbleTailImage, 0, 0, speechBubbleTail.width, speechBubbleTail.height, boundingBox.x1 + x + 5, boundingBox.y1 + y - bubblePadding - 1, 8, 8);
    context.restore();
}
