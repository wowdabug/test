...
const premiumMinerMessage = _("Buy a 650 or 1400 ticket pack and get your name in the mine like I did!")
...
function addPremiumQuoteFromDoubleClick(workerWorldDepth, workerNum)
{
    if(!areQuotesEnabled || workerWorldDepth == depth + 1) return;
    for(var i = activeMinerTexts.length - 1; i >= 0; i--)
    {
...
{
    return (battleManager.battleWaiting.length > 0 && Math.abs(battleManager.battleWaiting[0][1] - queriedDepth) < 2);
}
...
{
    return (isMobile()) ? queriedDepth == 1089 && minerNum == 7 : queriedDepth == 1089 && minerNum == 8;
}
