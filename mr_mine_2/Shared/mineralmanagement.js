...
const WEAPON_SCRAP_INDEX = 69;
const RELIC_SCRAP_INDEX = 70;
...
class WorldResource 
{
    ...
    add(amount, source)
    {
        if(this.totalOwnedEver[source] == undefined)
        {
            this.totalOwnedEver[source] = 0;
        }

        this.totalOwnedEver[source] += amount;
        this.totalOwnedEver.total += amount;
        this.numOwned += amount;
    }
}
...
buildingMaterials.resetsWithAscension = false;
...
sulfur.largeIconHidden = sulpherIconHiddenHD;
sulfur.isTradable = true;
...
nickel.sellValue = new BigNumber(75000000000000000);
...
alexandrite.sellValue = new BigNumber(1250000000000000000);
...
benitoite.sellValue = new BigNumber(12500000000000000000);
...
cobalt.sellValue = new BigNumber(60000000000000000000);
...
superMinerSouls.resetsWithAscension = false;
...
var weaponScrap = new WorldResource();
weaponScrap.index = WEAPON_SCRAP_INDEX;
weaponScrap.name = _("Weapon Scrap");
weaponScrap.sellValue = new BigNumber(0);
weaponScrap.isIsotope = false;
weaponScrap.isOnHeader = false;
weaponScrap.isotopeIndex = 0;
weaponScrap.indexOfWorldEncountered = EARTH_INDEX;
weaponScrap.smallIcon = weaponScrapSmall;
weaponScrap.smallIconHidden = weaponScrapSmall;
weaponScrap.largeIcon = weaponScrapHD;
weaponScrap.largeIconHidden = weaponScrapHD;
weaponScrap.isTradable = false;
weaponScrap.resetsWithAscension = false;
worldResources[WEAPON_SCRAP_INDEX] = weaponScrap;

var relicScrap = new WorldResource();
relicScrap.index = RELIC_SCRAP_INDEX;
relicScrap.name = _("Relic Scrap");
relicScrap.sellValue = new BigNumber(0);
relicScrap.isIsotope = false;
relicScrap.isOnHeader = false;
relicScrap.isotopeIndex = 0;
relicScrap.indexOfWorldEncountered = EARTH_INDEX;
relicScrap.smallIcon = relicScrap;
relicScrap.smallIconHidden = relicScrap;
relicScrap.largeIcon = relicScrapHD;
relicScrap.largeIconHidden = relicScrapHD;
relicScrap.isTradable = true;
worldResources[RELIC_SCRAP_INDEX] = relicScrap;
...
function estimateTotalMineralsMinedPerSecond(useSuperMinerMultiplier = true, useMineralSpeedMultiplier = true)
{
    if(estimateTotalMineralsRequiresCacheUpdate())
    {
        cachedMinerSpeed = useMineralSpeedMultiplier ? STAT.minerSpeedMultiplier() : 1;
...
function valueOfMineralsPerSecond(useSuperMinerMultiplier = true, useMineralSpeedMultiplier = true)
{
    ...
    var mineralsMined = estimateTotalMineralsMinedPerSecond(useSuperMinerMultiplier, useMineralSpeedMultiplier);
...
function addWeaponScrap(amount)
{
    worldResources[WEAPON_SCRAP_INDEX].numOwned += amount;
}

function addRelicScrap(amount)
{
    worldResources[RELIC_SCRAP_INDEX].numOwned += amount;
}

function removeRelicScrap(amount)
{
    worldResources[RELIC_SCRAP_INDEX].numOwned -= amount;
}
//############################################################
...
function numRelicScrapsOwned()
{
    return worldResources[RELIC_SCRAP_INDEX].numOwned;
}

//ascension logic

function resetWorldResourcesForAscension()
{
    worldResources.forEach(resource =>
    {
        if(resource.resetsWithAscension)
        {
            resource.numOwned = 0;
        }
    })
