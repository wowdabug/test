...
function onWorkerClicked(d, workerNum)
{
    ...
    else if(battleManager.battleWaiting.length > 0 && battleManager.battleWaiting[0][0] == workerNum && battleManager.battleWaiting[0][1] == depthToCheck)
    {
        battleManager.preparebattle();
        depthOfMonster = battleManager.battleWaiting[0][1];
    }
    else
    {
        addWorkerQuoteFromClick(d, workerNum)
...
function onWorkerDoubleClicked(d, workerNum)
{
    var depth = (currentlyViewedDepth - (5 - d));
    var workerIndex = workerNum - 1;

    var isPremium = getPremiumMinerName(depth, workerIndex);

    if(isPremium)
    {
        addPremiumQuoteFromDoubleClick(depth, workerNum);
    }
}