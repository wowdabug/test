class ReactorWindow extends TabbedPopupWindow
{
    ...
    loadReactorButtonClicked(index)
    {
        return function ()
        {
            var remainingFuelCells = 0;
            reactor.grid.fuelCellRemainingEnergy.forEach((fuelCell) =>
            {
                let componentTypeAtCell = reactor.grid.grid[fuelCell.y][fuelCell.x];
                if(fuelCell.remainingEnergy != reactorComponents[componentTypeAtCell].totalEnergyOutput)
                {
                    remainingFuelCells++;
                }
            })

            if(remainingFuelCells > 0)
            {
        ...
        for(var i = 0; i < blueprintsInSubcategory.length; ++i)
        {
            ...
            var itemHitbox = new Hitbox(
                ...
                },
                {
                    ...
                    {
                        console.log("Selected blueprint: " + blueprint.craftedItem.item.getName());
