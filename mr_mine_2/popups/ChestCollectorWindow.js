class ChestCollectorWindow extends TabbedPopupWindow
{
    ...
    domElementId = "COLLECTD"; // ID of dom element that gets shown or hidden
    context = CR;         // Canvas rendering context for popup
    ...
    initializeBaseTab()
    {
        ...
        let chestWidth = this.bodyContainer.boundingBox.width * 0.22;
        let chestHeight = this.bodyContainer.boundingBox.height * 0.7;
        let padding = this.bodyContainer.boundingBox.width * 0.04;
        //basic chest
        ...
        //battle chest
        this.compressorPane.battleChestDisplay = new Hitbox(
            {
                x: (chestWidth + (padding)) * 3,
                y: this.bodyContainer.boundingBox.height * -0.05,
                width: chestWidth,
                height: chestHeight
            }, {}, ""
        );
        this.compressorPane.battleChestDisplay.render = function (parentWindow)
        {
            var coords = this.getRelativeCoordinates(0, 0, parentWindow);
            var imageBoundingBox = drawImageFitInBox(
                parentWindow.context,
                battleChestIconClosed,
                coords.x,
                coords.y,
                this.boundingBox.width,
                this.boundingBox.width
            ...
            fillTextWrap(
                parentWindow.context,
                "x " + chestService.getStoredChestsOfType(ChestType.battle),
                coords.x,
                imageBoundingBox.y + imageBoundingBox.height,
                this.boundingBox.width,
                "center"
            ...
            parentWindow.context.restore();
            this.renderChildren();
        }.bind(this.compressorPane.battleChestDisplay, this);
        this.compressorPane.addHitbox(this.compressorPane.battleChestDisplay);

        this.compressorPane.battleChestButton = this.compressorPane.addHitbox(new Button(
            upgradebg_blank, _("Open Battle Chest"), "18px KanitM", "#000000",
            {
                x: this.compressorPane.battleChestDisplay.boundingBox.x,
                y: this.compressorPane.battleChestDisplay.boundingBox.height * 0.7,
                width: this.compressorPane.battleChestDisplay.boundingBox.width,
                height: this.compressorPane.battleChestDisplay.boundingBox.height * 0.15
            },
            {
                onmousedown: function ()
                {
                    chestService.grantStoredChest(ChestType.battle);
                }
            }
        ));
    ...
    render()
    {
        ...
        if(this.currentTabIndex == 0)
        {
            ...
            this.compressorPane.battleChestButton.image = chestService.getStoredChestsOfType(ChestType.battle) > 0 ? upgradeb : upgradebg_blank;
